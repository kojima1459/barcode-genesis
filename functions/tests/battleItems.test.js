...
test("BOOST applies once and scales the first hit damage", () => {
  ...
  const boostResult = simulateBattle(robot1, robot2, battleId, [], undefined, { p1: "BOOST" });
...
test("SHIELD applies once and scales the first incoming hit", () => {
  const robot1 = createRobot({ id: "r1", name: "R1", baseSpeed: 10 });
  const robot2 = createRobot({ id: "r2", name: "R2", baseSpeed: 8 });
  const battleId = "item-shield-1";

  const baseResult = simulateBattle(robot1, robot2, battleId);
  const shieldResult = simulateBattle(robot1, robot2, battleId, [], undefined, { p2: "SHIELD" });
  ...
  const shieldEvents = shieldResult.logs.filter((log) => log.itemType === "SHIELD");
  assert.equal(shieldEvents.length, 1);
});

test("JAMMER cancels the next critical hit once", () => {
  const robot1 = createRobot({ id: "r1", name: "R1", baseSpeed: 10 });
  const robot2 = createRobot({ id: "r2", name: "R2", baseSpeed: 8 });

  const battleId = findCriticalBattleId(robot1, robot2);
  assert.ok(battleId, "expected to find a battleId with an opening critical hit");

  const baseResult = simulateBattle(robot1, robot2, battleId);
  assert.equal(baseResult.logs[0]?.isCritical, true);

  const jamResult = simulateBattle(robot1, robot2, battleId, [], undefined, { p2: "JAMMER" });
